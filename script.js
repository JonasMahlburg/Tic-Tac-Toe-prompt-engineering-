...
function render() {
    let playerIndicator = document.getElementById('current-player');
    playerIndicator.innerHTML = `Aktueller Spieler: ${currentPlayer === 'circle' ? generateCircleSVG() : generateCrossSVG()}`;
...
function drawWinningLine(pattern) {
    ...
    const svgLine = `
        <svg width="300" height="300" style="position:absolute; top:0; left:0;">
            <line x1="${startCoord.x}" y1="${startCoord.y}" x2="${endCoord.x}" y2="${endCoord.y}" stroke="white" stroke-width="4">
...
function updateWinCounter() {
    ...
    winCounter.innerHTML = `Spieler 1 (Kreis): ${wins.circle} Siege<br>Spieler 2 (Kreuz): ${wins.cross} Siege`;
}

// Beispiel, wie du den Gewinner hinzufügen kannst
